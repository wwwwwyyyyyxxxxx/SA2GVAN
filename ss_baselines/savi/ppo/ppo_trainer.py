...
from ss_baselines.common.utils import (batch_obs, generate_video, linear_decay, plot_top_down_map, resize_observation, NpEncoder)
...
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        if not ppo_cfg.use_external_memory:
            ...
            self.actor_critic = AudioNavSMTPolicy(observation_space=observation_space,
    ...
    def _extract_scalars_from_info(cls, info: Dict[str, Any]) -> Dict[str, float]:
        ...
        for k, v in info.items():
            ...
            if isinstance(v, dict):
                result.update({k + "." + subk: subv for subk, subv in cls._extract_scalars_from_info(v).items() if (k + "." + subk) not in cls.METRICS_BLACKLIST})
    ...
    def train_belief_predictor(self, rollouts):
        ...
        for e in range(num_epoch):
            ...
            for sample in data_generator:
                ...
                masks = (torch.sum(torch.reshape(obs_batch[SpectrogramSensor.cls_uuid], (obs_batch[SpectrogramSensor.cls_uuid].shape[0], -1)), dim=1, keepdim=True) != 0).float()
                ...
                bitwise_close = torch.bitwise_and(torch.isclose(rounded_preds[:, 0], transformed_gts[:, 0]), torch.isclose(rounded_preds[:, 1], transformed_gts[:, 1]))
    ...
    def _eval_checkpoint(self, checkpoint_path: str, writer: TensorboardWriter, checkpoint_index: int = 0) -> Dict:
        ...
        batch = batch_obs(obs_copy, self.device, skip_list=['view_point_goals', 'intermediate', 'oracle_action_sensor'])
        ...
        while (len(stats_episodes) < self.config.TEST_EPISODE_COUNT and self.envs.num_envs > 0):
            ...
            with torch.no_grad():
                _, actions, _, test_recurrent_hidden_states, test_em_features = self.actor_critic.act(batch,
                ...
                if not_done_masks[i].item() == 0:
                    ...
                    if "top_down_map" in self.config.VISUALIZATION_OPTION:
                        if self.config.RL.PPO.use_belief_predictor:
                            ...
                            pred = None
                        top_down_map = plot_top_down_map(infos[i], dataset=self.config.TASK_CONFIG.SIMULATOR.SCENE_DATASET, pred=pred)
                        scene = current_episodes[i].scene_id.split('/')[3]
                        sound = current_episodes[i].sound_id.split('/')[1][:-4]
                        writer.add_image(f"{config.EVAL.SPLIT}_{scene}_{current_episodes[i].episode_id}_{sound}/" f"{infos[i]['spl']}", top_down_map, dataformats='WHC')
            ...
            (
                ...
                batch,
                rgb_frames,
            ) = self._pause_envs(envs_to_pause, self.envs, test_recurrent_hidden_states, not_done_masks, current_episode_reward, prev_actions, batch, rgb_frames, test_em, descriptor_pred_gt)
